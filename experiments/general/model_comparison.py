...
# Store unnormalized targets for sklearn models
...
mean_np = mean.squeeze(0).squeeze(0).cpu().numpy()  # (features,)
std_np = std.squeeze(0).squeeze(0).cpu().numpy()    # (features,)

# Use the already-normalized torch tensors to avoid re-normalizing by hand
...
# Import classes by reading the files and extracting the classes
# We'll define the necessary classes inline to avoid import issues
from typing import Optional, Tuple, List
...
print("\n" + "="*60)
print("Generating simplified comparison plots...")
print("="*60)
...
axes[0].set_title("Model RMSE (Original Scale)")
...
ax.set_title("Neural Models Test RMSE Convergence (Original Scale)")
ax.legend()
ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig("neural_convergence.png", dpi=150, bbox_inches='tight')
print("Saved: neural_convergence.png")
