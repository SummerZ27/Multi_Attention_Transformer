"""
Sinwave Data Comparison Experiment
Compares Naive Transformer, PTA Transformer, PatchTST, and Hierarchical PTA on sinwave.csv
...
class SinwavePredictor(nn.Module):
    """Naive Transformer for sinwave prediction"""
...
class SinwavePredictorPTA(nn.Module):
    """PTA Transformer for sinwave prediction"""
...
class SinwavePredictorHierPTA(nn.Module):
    """Hierarchical PTA Transformer for sinwave prediction"""
    def __init__(self, input_dim: int, dim: int, attn_dim: int, mlp_dim: int, num_heads: int, num_layers: int,
...
class PatchTSTWrapper(nn.Module):
    """Wrapper for PatchTST to work with sinwave data format"""
...
def load_sinwave_data(csv_path: str = "sinwave.csv"):
    """Load sinwave.csv and return DataFrame"""
    df = pd.read_csv(csv_path)
    return df

def prepare_sinwave_data(df: pd.DataFrame, window: int = 96, target_col: str = "x_t"):
    """
    Prepare sinwave data for training
    Uses single feature column as input
    Predicts target column
    """
    # Get feature columns
    feature_cols = [col for col in df.columns if col != 't']  # Exclude time column
    ...
    features = df[feature_cols].values.astype(np.float32)  # (N, num_features)
...
print("\n" + "="*80)
print("SINWAVE DATA COMPARISON EXPERIMENT")
print("="*80)
...
print("Features: x_t")
print("\nAll models:")
print("  - Use x_t as input")
print("  - Predict x_t")
print("  - Same horizon=96 for fair comparison")

# Load sinwave data
print("\n" + "="*80)
print("Loading sinwave data...")
print("="*80)
df = load_sinwave_data("sinwave.csv")
print(f"Loaded sinwave data: {len(df)} rows, {len(df.columns)} columns")
print(f"Date range: t={df['t'].min()} to t={df['t'].max()}")
print(f"All models use x_t as input and predict x_t")
...
# Prepare data
train_loader, val_loader, test_loader, y_mean, y_std, num_features = prepare_sinwave_data(df, window=WINDOW, target_col="x_t")
print(f"\nData splits:")
print(f"  Train: {len(train_loader.dataset)} samples")
print(f"  Val: {len(val_loader.dataset)} samples")
print(f"  Test: {len(test_loader.dataset)} samples")
print(f"  Input features: {num_features}")
print(f"  Target: x_t - same for all models")
...
model4 = SinwavePredictorHierPTA(
    input_dim=num_features,
    dim=128,
    attn_dim=64,
    mlp_dim=256,
...
ax.set_ylabel("RMSE")
ax.set_title("RMSE Comparison - Sinwave Data (Horizon=96)")
ax.set_xticks(x)
...
ax.set_ylabel("MAE")
ax.set_title("MAE Comparison - Sinwave Data (Horizon=96)")
ax.set_xticks(x)
ax.set_xticklabels(models, rotation=45, ha='right')
ax.grid(axis='y', alpha=0.3)
...
ax.set_title("Validation RMSE Convergence - Sinwave Data (Horizon=96)")
ax.set_xlim(1, 100)
...
ax.set_ylabel("Error")
ax.set_title("MSE vs MAE Comparison - Sinwave Data (Horizon=96)")
ax.set_xticks(x)
ax.set_xticklabels(models, rotation=45, ha='right')
ax.legend()
ax.grid(axis='y', alpha=0.3)
...
plt.tight_layout()
plt.savefig("sinwave_comparison_transformer.png", dpi=150, bbox_inches='tight')
print("Saved: sinwave_comparison_transformer.png")
...
plt.title("Sinwave Data Comparison: Transformer Models (Horizon=96, Target=x_t)", 
          fontsize=14, fontweight='bold', pad=20)
plt.savefig("sinwave_comparison_table.png", dpi=150, bbox_inches='tight')
print("Saved: sinwave_comparison_table.png")

print("\n" + "="*80)
print("Sinwave comparison experiment completed successfully!")
print("="*80)
