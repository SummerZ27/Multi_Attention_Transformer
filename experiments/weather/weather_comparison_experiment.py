"""
Weather Data Comparison Experiment
Compares Naive Transformer, PTA Transformer, and PatchTST on weather.csv
...
# ========== Model Definitions (Same as fair_comparison_experiment.py) ==========
...
class WeatherPredictor(nn.Module):
    """Naive Transformer for weather prediction"""
...
class WeatherPredictorPTA(nn.Module):
    """PTA Transformer for weather prediction"""
...
class PatchTSTWrapper(nn.Module):
    """Wrapper for PatchTST to work with weather data format"""
    def __init__(self, input_dim: int, seq_len: int, pred_len: int = 1, patch_len: int = 16, stride: int = 8, target_feature_idx: int = 1):
        super().__init__()
        self.seq_len = seq_len
        self.pred_len = pred_len
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Extract the target feature (temperature)
...
def load_weather_data(csv_path: str = "weather.csv"):
    """Load weather.csv and return DataFrame"""
...
def prepare_weather_data(df: pd.DataFrame, window: int = 96, target_col: str = "T (degC)"):
    """
    Prepare weather data for training
    Uses all features except Date Time as input (including target for PatchTST)
...
print("\n" + "="*80)
print("WEATHER DATA COMPARISON EXPERIMENT")
print("="*80)
...
print("\nDataset: weather.csv")
print("Target: Temperature (T (degC))")
print("Features: All 21 weather features (including target in input for PatchTST)")
print("\nAll models:")
print("  - Use all 21 features as input")
print("  - Predict Temperature (T (degC))")
...
print(f"Loaded weather data: {len(df)} rows, {len(df.columns)} columns")
print(f"Date range: {df['Date Time'].min()} to {df['Date Time'].max()}")
print(f"Total features: {len(df.columns) - 1} (excluding Date Time)")
print(f"All models use all {len(df.columns) - 1} features as input and predict Temperature")
...
# Prepare data
train_loader, val_loader, test_loader, y_mean, y_std, num_features = prepare_weather_data(df, window=WINDOW)
print(f"\nData splits:")
print(f"  Train: {len(train_loader.dataset)} samples")
print(f"  Val: {len(val_loader.dataset)} samples")
print(f"  Test: {len(test_loader.dataset)} samples")
print(f"  Input features: {num_features} (all features including temperature)")
print(f"  Target: Temperature (T (degC)) - same for all models")
...
ax.set_ylabel("RMSE")
ax.set_title("RMSE Comparison - Weather Data (Horizon=96)")
ax.set_xticks(x)
...
ax.set_ylabel("MAE")
ax.set_title("MAE Comparison - Weather Data (Horizon=96)")
ax.set_xticks(x)
ax.set_xticklabels(models, rotation=45, ha='right')
ax.grid(axis='y', alpha=0.3)
...
ax.set_title("Validation RMSE Convergence - Weather Data (Horizon=96)")
ax.set_xlim(1, 100)
...
ax.set_ylabel("Error")
ax.set_title("MSE vs MAE Comparison - Weather Data (Horizon=96)")
ax.set_xticks(x)
ax.set_xticklabels(models, rotation=45, ha='right')
ax.legend()
ax.grid(axis='y', alpha=0.3)
...
plt.tight_layout()
plt.savefig("weather_comparison_transformer.png", dpi=150, bbox_inches='tight')
print("Saved: weather_comparison_transformer.png")
...
plt.title("Weather Data Comparison: Transformer Models (Horizon=96, Target=Temperature)", 
          fontsize=14, fontweight='bold', pad=20)
plt.savefig("weather_comparison_table.png", dpi=150, bbox_inches='tight')
print("Saved: weather_comparison_table.png")

print("\n" + "="*80)
print("Weather comparison experiment completed successfully!")
print("="*80)
